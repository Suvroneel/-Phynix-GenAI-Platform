...
This is the hybrid BERT + GenAI chat interface for Phynix.

ARCHITECTURE FLOW:
1. User types message
2. BERT analyzes emotion (sadness, joy, anger, etc.) - KEPT FOR ANALYTICS
3. Calculate risk level (High/Moderate/Low/Neutral) - KEPT FOR GRAPHS
4. GenAI generates response using emotion context - NEW! REPLACES HARD-CODED REPLIES
5. Save everything to database (message + emotion + risk + AI reply)
6. Display AI response with collapsible emotion analysis

WHAT CHANGED FROM ORIGINAL Chat.py:
- REMOVED: Hard-coded response dictionaries (emotion_responses)
- REMOVED: random.choice() picking template replies
- ADDED: gen_ai.PhynixAI integration
- ADDED: Model selector UI (user can switch AI models)
- ADDED: Context-aware AI responses based on detected emotion

WHAT STAYED THE SAME:
- BERT emotion detection (predict_emotion) - ESSENTIAL for analytics
- Risk level calculation (get_risk_with_emoji) - ESSENTIAL for dashboards
- Database saving (Supabase) - All data still tracked for graphs
- UI/UX design - Chat bubbles, analysis block, styling
- Authentication flow - Login/session management
- Welcome messages - First-time user vs returning user

WHY HYBRID APPROACH:
- BERT provides consistent emotion classification for trend analysis
- GenAI provides natural, varied responses for better UX
- Best of both worlds: reliable analytics + engaging conversations

DATABASE SCHEMA (unchanged):
- messages: User input
- predicted_emotion: BERT classification (7 emotions)
- risks: Risk level from emotion mapping
- replies: NOW contains GenAI response instead of template

FUTURE: This can be deployed alongside Chat.py for A/B testing
"""
